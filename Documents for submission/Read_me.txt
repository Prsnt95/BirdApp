##-0- Configure your email to receieve emails from our app

    - Email "prashantghimire2026@u.northwestern.edu" to get verified and follow the instructions in the email. 
	##SES free tier requires emails to be verified to receive emails. 
## You can also try to verify your email by yourself with the steps below: 
a-Sign in to the AWS Management Console:
b-Go to AWS SES Console.
c-Choose Your Region: Ohio-us-east-2
d- Navigate to Identities under the SES sidebar menu, under Configuration
e- Click Create identity > Select Email address.
f- Enter the email address and click Verify.
g-Check your inbox for a verification email and click the provided link to verify the email address.



##-1-Make sure you have python 3.12 installed and install the packages below:

    pip install requests
    pip install jsons
    pip install uuid
    pip install pathlib
    pip install logging
    pip install sys
    pip install os
    pip install base64
    pip install time
    pip install configparser
    pip install geoapify
    pip install python-dotenv
    pip install boto3
    pip install configparser
    pip install python-dotenv



##-2-start anaconda terminal 

##-3-type the command below:

	cd "folder path"

##where "folder path" is the location of the artifact files

##-4- run the command below: 
	python db_test.python
	
## follow the instructions by the app.

